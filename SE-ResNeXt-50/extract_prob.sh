...
extract_features() {
  ...
  for num in 0 ; do
    ...
    local model=/mnt/lustre/yangkunlin/kaggle_camera/SE-ResNeXt-50/temp_SE-ResNeXt-50_test.prototxt
    local list=test${num}
    sed -e "s/\${list}/${list}/g" \
      ${MODELS_DIR}/SE-ResNeXt-50_test.prototxt > ${model}
    echo ${model}
...
trained_model=/mnt/lustre/yangkunlin/kaggle_camera/SE-ResNeXt-50/model/SE-ResNeXt-50_iter_35000.caffemodel

...
MODELS_DIR=/mnt/lustre/yangkunlin/kaggle_camera/resnet50

extract_features() {

  local trained_model=$1

  local result_dir=/mnt/lustre/yangkunlin/kaggle_camera/resnet50/model/result
  # rm -rf ${result_dir}
  # mkdir -p ${result_dir}
  echo ${result_dir}
  ...
  for num in 0 ; do
    ...
    local num_iters=$(((num_samples + 16) / 17))
    local model=/mnt/lustre/yangkunlin/kaggle_camera/resnet50/temp_resnet50_test.prototxt
    local list=test${num}
    sed -e "s/\${list}/${list}/g" \
      ${MODELS_DIR}/resnet50_test.prototxt > ${model}
    echo ${model}
    MV2_USE_CUDA=1 MV2_ENABLE_AFFINITY=0 MV2_SMP_USE_CMA=0 \
      ...
      ${num_iters} lmdb GPU
    srun -p test python2 /mnt/lustre/yangkunlin/kaggle_camera/resnet50/convert_lmdb_to_numpy.py \
...
trained_model=/mnt/lustre/yangkunlin/kaggle_camera/resnet50/model/resnet50_iter_5000.caffemodel
